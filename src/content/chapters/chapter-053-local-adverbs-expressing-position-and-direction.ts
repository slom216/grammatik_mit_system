import type { ChapterDefinition } from '../../schemas/chapterSchema';

export const chapter053: ChapterDefinition = {
  id: 'chapter-053',
  number: 53,
  slug: 'local-adverbs-expressing-position-and-direction',
  title: 'Local Adverbs Expressing Position and Direction',
  germanTitle: 'Lokaladverbien: hier, da, dort, hin und her',
  level: 'A2',
  section: 'words-and-word-formation',
  objective:
    'Use hier, da, and dort to describe a fixed location, choose correctly between hin (away from the speaker) and her (toward the speaker), form and use the compounds hierher, dorthin, daher, and dahin, and tell the pure location adverbs drinnen, draußen, oben, and unten apart from their directional counterparts.',
  prerequisites: [8, 37, 38, 39],
  estimatedMinutes: 24,
  tags: [
    'adverbs',
    'lokaladverbien',
    'hier',
    'da',
    'dort',
    'hin',
    'her',
    'hierher',
    'dorthin',
    'daher',
    'dahin',
    'drinnen',
    'draußen',
    'oben',
    'unten',
    'speaker perspective',
    'dialogue',
  ],
  explanation: {
    introduction: [
      'You already know how to ask wo? (where — a location), wohin? (where to — a destination), and woher? (where from — an origin) and answer them with prepositions like in, zu, and aus. German also has a set of simple adverbs that answer exactly the same three questions without naming a place at all: hier, da, dort for location, and hin, her, and their compounds for direction.',
      'The trickiest part of this system is not vocabulary but perspective: hin and her do not describe distance or size, only which way something moves relative to the person speaking. The same walk across a room is her for someone waving you closer and hin for someone sending you away — which is exactly why this chapter leans heavily on short dialogues, since you often cannot tell hin from her without knowing where the speaker is standing.',
    ],
    rules: [
      {
        id: 'ch53-rule-01',
        heading: 'hier, da, and dort: three ways to say "here" and "there"',
        paragraphs: [
          'German has three basic adverbs for a place without naming it. hier marks the spot where the speaker is standing right now. dort marks a place clearly separate from both speaker and listener, often something you could point at from a distance. da sits between the two: it is used for a place that is nearby, already mentioned, or not sharply distinguished from hier. Ich bin hier. (I am here.) Siehst du das Schloss dort drüben? (Do you see the castle over there?) Ist da jemand? (Is someone there?)',
          'da also has a very common idiomatic use where it barely names a place at all, only presence: Ich bin gleich wieder da. (I\'ll be right back — literally "there/here again", meaning "available/present".)',
          'Because da and dort overlap in meaning, dort is the safer choice whenever distance or separation from the speaker genuinely matters, while da fits something close at hand, already known from context, or fixed idioms like da sein.',
        ],
      },
      {
        id: 'ch53-rule-02',
        heading: 'hin and her: direction relative to the speaker',
        paragraphs: [
          'Unlike hier, da, and dort, which describe a fixed location, hin and her describe direction — always relative to the speaker (or, in a story, to whoever the narrative is centered on). her means "toward the speaker": Komm her! (Come here, to me!). hin means "away from the speaker": Geh hin! (Go there, away from me!).',
          'This distinction is about perspective, not distance: the same short trip from the kitchen to the garden is her for the person standing in the garden calling someone over, and hin for the person in the kitchen being sent outside.',
          'hin and her regularly attach to verbs of motion as separable prefixes, just like the separable verbs you already know: hingehen (to go there), herkommen (to come here), hinfahren, herbringen. In relaxed spoken German, hin and her are also often split off from wohin? and woher? and moved to the very end of the sentence: Wo gehst du hin? (= Wohin gehst du?) and Wo kommst du her? (= Woher kommst du?).',
        ],
      },
      {
        id: 'ch53-rule-03',
        heading: 'Compounds: hierher, dorthin, daher, dahin',
        paragraphs: [
          'hier, da, and dort combine with hin and her to build four very common compound adverbs that pack both a location and a direction into a single word: hierher (to here, toward the speaker), dorthin (to there, away from the speaker), daher (from there, toward the speaker), and dahin (to there, away from the speaker).',
          "Kannst du bitte hierher kommen? (Can you please come here?) Wir fahren morgen dorthin, wo meine Großeltern wohnen. (We're driving there tomorrow, to where my grandparents live.) Notice that the second half of the compound, not the first, tells you the direction: hierher and daher both point toward the speaker because they end in -her, while dorthin and dahin both point away because they end in -hin. One caution: daher in this local sense (from there) sounds rather old-fashioned today. Everyday German usually says von da or da … her instead (Da komme ich gerade her.), and daher itself most often means therefore.",
          'dahin and dorthin are close in meaning and often interchangeable; dahin leans on da (a place already established in the conversation), while dorthin leans on dort (a place understood as more clearly separate). Both are far more natural in everyday speech than a bare hin or her standing with no location word at all.',
        ],
      },
      {
        id: 'ch53-rule-04',
        heading: 'drinnen, draußen, oben, unten: further position adverbs',
        paragraphs: [
          "Four more adverbs round out the system for describing where something is (wo?), with no sense of movement: drinnen (inside), draußen (outside), oben (up, upstairs, above), and unten (down, downstairs, below). Draußen ist es kalt, aber drinnen ist es warm. (It's cold outside, but it's warm inside.) Meine Wohnung ist ganz oben, der Keller ist unten. (My apartment is right at the top, the cellar is downstairs.)",
          'Like hier, da, and dort, these four are pure location adverbs — they answer wo?, not wohin?. To turn them into a destination, German adds nach (nach draußen, nach oben) rather than using the plain adverb with a verb of motion.',
          'You will also meet dedicated hin-/her- compound verb prefixes built on the same pattern for direction — hinaus/heraus (out), hinein/herein (in), hinauf/herauf (up), hinunter/herunter (down) — but at this level it is enough to recognize that a bare drinnen, draußen, oben, or unten never expresses movement by itself.',
        ],
      },
      {
        id: 'ch53-rule-05',
        heading: 'Word order: where hin, her, and their compounds go',
        paragraphs: [
          'In relaxed spoken German, hin and her are very often detached from wohin? and woher? and moved to the end of the sentence, leaving a bare wo? at the front: Wo gehst du hin? and Wo kommst du her?. Both the split and the intact form (Wohin gehst du?, Woher kommst du?) are correct; the split form is extremely common in everyday conversation.',
          "The compounds hierher, dorthin, daher, and dahin behave like ordinary adverbs and can also open the sentence for emphasis, taking the first position before the verb: Dorthin fahre ich nie wieder! (I'm never driving there again!) Hierher kommt so schnell keiner. (Nobody gets here that fast.)",
          'A bare hin or her, however, is not used alone at the front of a sentence — it needs to be attached to a verb (hingehen, herkommen) or combined with a location word first, as in the compounds above.',
        ],
      },
      {
        id: 'ch53-rule-06',
        heading: 'Choosing hin vs. her from context and dialogue',
        paragraphs: [
          "Because hin and her depend entirely on the speaker's own position, you often cannot tell which one is correct from a single isolated sentence — you need to know where the speaker is standing relative to the place or person being talked about.",
          'In conversation, this becomes clear from what was just said. If someone tells you they are somewhere and asks you to join them, her (or a her-compound like hierher) fits, since you would be moving toward them. If someone asks you to go check on something happening elsewhere, hin (or a hin-compound like hin, dorthin, or dahin) fits, since the motion described moves away from the current speaker.',
          'Practicing hin and her inside short dialogues, rather than single sentences, is the best way to build a feel for this — many of the exercises in this chapter use short exchanges for exactly that reason.',
        ],
      },
      {
        id: 'ch53-rule-07',
        heading: 'Connecting back to wo?, wohin?, and woher?',
        paragraphs: [
          "This chapter's adverbs answer the same three question words you already know from Chapters 37–39. hier, da, dort, drinnen, draußen, oben, and unten answer wo? (Wo bist du? — Ich bin hier. / Ich bin draußen.). dorthin, hierher, and hin/her themselves answer wohin? or woher? (Wohin gehst du? — Ich gehe dorthin. Woher kommst du? — Ich komme daher.)",
          'Just as with the prepositions in Chapters 37–39, the underlying logic never changes: a fixed location (wo?), a destination (wohin?), or an origin (woher?) — only now the answer is a single adverb instead of a preposition plus a noun.',
        ],
      },
    ],
    tables: [
      {
        id: 'ch53-table-01',
        title: 'Location adverbs: hier, da, dort, drinnen, draußen, oben, unten',
        columns: ['Adverb', 'Meaning', 'Example'],
        rows: [
          ['hier', "here (at the speaker's location)", 'Ich bin hier.'],
          ['da', 'there (nearby, or already mentioned)', 'Ist da jemand?'],
          ['dort', 'there (farther away, clearly separate)', 'Dort drüben ist die Post.'],
          ['drinnen', 'inside', 'Die Kinder spielen drinnen.'],
          ['draußen', 'outside', 'Es regnet draußen.'],
          ['oben', 'up / upstairs / above', 'Meine Eltern sind oben.'],
          ['unten', 'down / downstairs / below', 'Der Keller ist unten.'],
        ],
        note: 'All seven adverbs answer wo? and describe a fixed location, never movement.',
      },
      {
        id: 'ch53-table-02',
        title: 'hin and her: patterns for direction relative to the speaker',
        columns: ['Pattern', 'Function', 'Example'],
        rows: [
          [
            'hin',
            'motion away from the speaker/reference point',
            'Geh hin und frag ihn!',
          ],
          ['her', 'motion toward the speaker/reference point', 'Komm sofort her!'],
          ['wo ... hin (split)', 'colloquial splitting of wohin?', 'Wo gehst du hin?'],
          ['wo ... her (split)', 'colloquial splitting of woher?', 'Wo kommst du her?'],
        ],
        note: 'hin and her can attach to a verb as a separable prefix (hingehen, herkommen) or stand at the end of a split question.',
      },
      {
        id: 'ch53-table-03',
        title: 'Compounds: combining location and direction',
        columns: ['Compound', 'Built from', 'Meaning', 'Example'],
        rows: [
          ['hierher', 'hier + her', 'to here (toward the speaker)', 'Komm hierher!'],
          ['dorthin', 'dort + hin', 'to there (away from the speaker)', 'Geh dorthin!'],
          [
            'daher',
            'da + her',
            'from there (toward the speaker); old-fashioned in this sense — today daher usually means "therefore"',
            'Er kommt genau daher.',
          ],
          [
            'dahin',
            'da + hin',
            'to there (away from the speaker)',
            'Der Weg führt dahin.',
          ],
        ],
        note: 'The second half of the compound (-her or -hin) always tells you the direction relative to the speaker.',
      },
    ],
    examples: [
      {
        german: 'Ich bin hier, komm zu mir!',
        english: 'I am here, come to me!',
        highlight: ['hier'],
        explanation: 'hier marks the exact spot where the speaker is standing.',
      },
      {
        german: 'Siehst du das Schloss dort drüben, ganz weit weg?',
        english: 'Do you see the castle over there, far away?',
        highlight: ['dort'],
        explanation:
          'dort is used for a place clearly separate from the speaker, often with a visible distance.',
      },
      {
        german: 'Es hat geklopft. Ist da jemand?',
        english: 'Someone knocked. Is anyone there?',
        highlight: ['da'],
        explanation:
          'da is the natural choice for a place that is close by or not sharply distinguished from hier.',
      },
      {
        german: 'Bleib hier, ich bin gleich wieder da!',
        english: "Stay here, I'll be right back!",
        highlight: ['da'],
        explanation:
          'da is used idiomatically here to mean "present/available", not a specific place.',
      },
      {
        german: 'Komm her, ich zeige dir etwas!',
        english: "Come here, I'll show you something!",
        highlight: ['her'],
        explanation: 'her marks motion toward the speaker.',
      },
      {
        german: 'Geh hin und frag ihn, wie spät es ist!',
        english: 'Go there and ask him what time it is!',
        highlight: ['hin'],
        explanation: 'hin marks motion away from the speaker, sending someone elsewhere.',
      },
      {
        german: 'Kannst du bitte hierher kommen? Ich brauche deine Hilfe.',
        english: 'Can you please come here? I need your help.',
        highlight: ['hierher'],
        explanation:
          "hierher combines hier and her: a destination that is the speaker's own location.",
      },
      {
        german: 'Wir fahren morgen dorthin, wo meine Großeltern wohnen.',
        english: 'We are driving there tomorrow, to where my grandparents live.',
        highlight: ['dorthin'],
        explanation:
          'dorthin combines dort and hin: a destination away from the speaker.',
      },
      {
        german: 'Er kommt genau daher, wo die Musik spielt.',
        english: 'He is coming exactly from where the music is playing.',
        highlight: ['daher'],
        explanation:
          'daher combines da and her: an origin, with motion toward the speaker.',
      },
      {
        german: 'Der Wanderweg führt genau dahin, wo der Wasserfall ist.',
        english: 'The hiking trail leads exactly there, to where the waterfall is.',
        highlight: ['dahin'],
        explanation: 'dahin combines da and hin: a destination away from the speaker.',
      },
      {
        german: 'Draußen ist es kalt, aber drinnen ist es warm.',
        english: 'It is cold outside, but it is warm inside.',
        highlight: ['draußen', 'drinnen'],
        explanation:
          'draußen and drinnen are pure location adverbs describing where something is, not movement.',
      },
      {
        german: 'Meine Wohnung ist ganz oben, der Keller ist unten.',
        english: 'My apartment is right at the top, the cellar is downstairs.',
        highlight: ['oben', 'unten'],
        explanation:
          'oben and unten describe vertical position, again with no sense of movement.',
      },
      {
        german: 'Wo gehst du hin? – Ich gehe ins Kino.',
        english: 'Where are you going? – I am going to the cinema.',
        highlight: ['hin'],
        explanation:
          'In casual speech, wohin? is often split, with hin moved to the end of the sentence.',
      },
      {
        german: 'Wo kommst du her? – Ich komme aus der Schule.',
        english: 'Where are you coming from? – I am coming from school.',
        highlight: ['her'],
        explanation: 'The same splitting happens with woher?, leaving her at the end.',
      },
    ],
    commonMistakes: [
      {
        incorrect: 'Komm hin!',
        correct: 'Komm her!',
        explanation:
          'When asking someone to approach you, use her (toward the speaker); hin would mean moving away, which contradicts the request to come closer.',
      },
      {
        incorrect: 'Geh her!',
        correct: 'Geh hin!',
        explanation:
          'When sending someone away from your own location, use hin; her would mean toward the speaker, the opposite of "go there".',
      },
      {
        incorrect: 'Komm dahin!',
        correct: 'Komm hierher!',
        explanation:
          'dahin points away from the speaker, so it cannot be combined with an invitation to approach; use a her-compound like hierher instead.',
      },
      {
        incorrect:
          'Dort ist mein Buch. (said while pointing at the book right next to the speaker)',
        correct: 'Hier ist mein Buch.',
        explanation:
          "dort implies a place clearly separate from the speaker; something within arm's reach should be hier, not dort.",
      },
      {
        incorrect: 'Wo gehst du her?',
        correct: 'Wo gehst du hin?',
        explanation:
          "her belongs to woher? (origin); hin belongs to wohin? (destination). Swapping them reverses the question's meaning.",
      },
      {
        incorrect: 'Er geht draußen.',
        correct: 'Er geht nach draußen.',
        explanation:
          'draußen is a pure location adverb (wo?); to express movement (wohin?) German needs nach draußen, not draußen alone.',
      },
    ],
    remember: [
      'hier = here (speaker\'s location); da = there (nearby or already mentioned, also "present" in da sein); dort = there (farther away, clearly separate).',
      'hin = away from the speaker; her = toward the speaker — both attach to verbs (hingehen, herkommen) and can split off to the end of a clause (Wo gehst du hin? / Wo kommst du her?).',
      'Compounds fuse location and direction: hierher (to here), dorthin (to there), daher (from there, toward the speaker; in this sense old-fashioned, today mostly "therefore"), dahin (to there, away from the speaker) — the ending -her or -hin always signals the direction.',
      'drinnen/draußen and oben/unten describe pure location (wo?); add nach to express a destination (nach draußen, nach oben).',
      'Context — usually who is speaking and from where — decides hin vs. her: the same journey can be hin for one speaker and her for another.',
    ],
  },
  mastery: {
    passingPercent: 80,
    minimumAnswered: 80,
    requiredCorrectTextInputs: 21,
    maxOpenReviewFlags: 3,
  },
  exercises: [
    {
      id: 'ch53-ex-01',
      chapterNumber: 53,
      order: 1,
      type: 'singleChoice',
      level: 'recognition',
      grammarFocus: ['hier', 'location'],
      instruction: 'Choose the correct adverb.',
      prompt: 'Ich bin ___, komm zu mir!',
      options: [
        { id: 'a', text: 'hier' },
        { id: 'b', text: 'da' },
        { id: 'c', text: 'dort' },
        { id: 'd', text: 'draußen' },
      ],
      correctOptionId: 'a',
      explanation: 'hier marks the exact spot where the speaker is standing.',
    },
    {
      id: 'ch53-ex-02',
      chapterNumber: 53,
      order: 2,
      type: 'singleChoice',
      level: 'recognition',
      grammarFocus: ['dort', 'location', 'distance'],
      instruction: 'Choose the correct adverb.',
      prompt: 'Siehst du das Schloss ___ drüben, ganz weit weg?',
      options: [
        { id: 'a', text: 'dort' },
        { id: 'b', text: 'hier' },
        { id: 'c', text: 'da' },
        { id: 'd', text: 'her' },
      ],
      correctOptionId: 'a',
      explanation:
        'dort is used for a place clearly separate from the speaker, especially with a visible distance ("drüben, weit weg").',
    },
    {
      id: 'ch53-ex-03',
      chapterNumber: 53,
      order: 3,
      type: 'singleChoice',
      level: 'recognition',
      grammarFocus: ['da', 'location'],
      instruction: 'Choose the correct adverb.',
      prompt: 'Es hat geklopft. Ist ___ jemand?',
      options: [
        { id: 'a', text: 'da' },
        { id: 'b', text: 'hier' },
        { id: 'c', text: 'dort' },
        { id: 'd', text: 'her' },
      ],
      correctOptionId: 'a',
      explanation:
        'Ist da jemand? is the standard phrase for asking whether someone is there, e.g. after a knock at the door.',
    },
    {
      id: 'ch53-ex-04',
      chapterNumber: 53,
      order: 4,
      type: 'singleChoice',
      level: 'recognition',
      grammarFocus: ['her', 'direction'],
      instruction: 'Choose the correct adverb.',
      prompt: 'Komm ___ zu mir, ich zeige dir etwas!',
      options: [
        { id: 'a', text: 'her' },
        { id: 'b', text: 'hin' },
        { id: 'c', text: 'da' },
        { id: 'd', text: 'dort' },
      ],
      correctOptionId: 'a',
      explanation: 'her marks motion toward the speaker, matching "zu mir" (to me).',
    },
    {
      id: 'ch53-ex-05',
      chapterNumber: 53,
      order: 5,
      type: 'singleChoice',
      level: 'recognition',
      grammarFocus: ['hin', 'direction'],
      instruction: 'Choose the correct adverb.',
      prompt: 'Geh ___ und frag ihn, wie spät es ist!',
      options: [
        { id: 'a', text: 'hin' },
        { id: 'b', text: 'her' },
        { id: 'c', text: 'da' },
        { id: 'd', text: 'dort' },
      ],
      correctOptionId: 'a',
      explanation: 'hin marks motion away from the speaker, sending someone elsewhere.',
    },
    {
      id: 'ch53-ex-06',
      chapterNumber: 53,
      order: 6,
      type: 'singleChoice',
      level: 'recognition',
      grammarFocus: ['hierher', 'compounds'],
      instruction: 'Choose the correct compound adverb.',
      prompt: 'Kannst du bitte ___ kommen? Ich brauche deine Hilfe.',
      options: [
        { id: 'a', text: 'hierher' },
        { id: 'b', text: 'dorthin' },
        { id: 'c', text: 'daher' },
        { id: 'd', text: 'dahin' },
      ],
      correctOptionId: 'a',
      explanation:
        "hierher (hier + her) is a destination that is the speaker's own location — the natural choice for calling someone over.",
    },
    {
      id: 'ch53-ex-07',
      chapterNumber: 53,
      order: 7,
      type: 'singleChoice',
      level: 'recognition',
      grammarFocus: ['dorthin', 'compounds'],
      instruction: 'Choose the correct compound adverb.',
      prompt: 'Wir fahren morgen ___, wo meine Großeltern wohnen.',
      options: [
        { id: 'a', text: 'dorthin' },
        { id: 'b', text: 'hierher' },
        { id: 'c', text: 'daher' },
        { id: 'd', text: 'her' },
      ],
      correctOptionId: 'a',
      explanation:
        'dorthin (dort + hin) is a destination away from the speaker, matching a place the grandparents live rather than the speaker.',
    },
    {
      id: 'ch53-ex-08',
      chapterNumber: 53,
      order: 8,
      type: 'singleChoice',
      level: 'recognition',
      grammarFocus: ['drinnen', 'draußen', 'location'],
      instruction: 'Choose the correct adverb.',
      prompt: 'Es regnet, deshalb bleiben wir lieber ___.',
      options: [
        { id: 'a', text: 'drinnen' },
        { id: 'b', text: 'draußen' },
        { id: 'c', text: 'oben' },
        { id: 'd', text: 'unten' },
      ],
      correctOptionId: 'a',
      explanation:
        'When it is raining, staying inside is drinnen; draußen would mean staying outside in the rain.',
    },
    {
      id: 'ch53-ex-09',
      chapterNumber: 53,
      order: 9,
      type: 'singleChoice',
      level: 'recognition',
      grammarFocus: ['oben', 'unten', 'location'],
      instruction: 'Choose the correct adverb.',
      prompt: 'Meine Wohnung liegt im obersten Stock, also ganz ___.',
      options: [
        { id: 'a', text: 'oben' },
        { id: 'b', text: 'unten' },
        { id: 'c', text: 'drinnen' },
        { id: 'd', text: 'draußen' },
      ],
      correctOptionId: 'a',
      explanation: 'The top floor (oberster Stock) corresponds to oben (up/at the top).',
    },
    {
      id: 'ch53-ex-10',
      chapterNumber: 53,
      order: 10,
      type: 'singleChoice',
      level: 'controlled',
      grammarFocus: ['hin', 'wohin-split'],
      instruction: 'Choose the correct word to complete the colloquial split question.',
      prompt: 'Wo gehst du ___? – Ich gehe ins Kino.',
      options: [
        { id: 'a', text: 'hin' },
        { id: 'b', text: 'her' },
        { id: 'c', text: 'da' },
        { id: 'd', text: 'dort' },
      ],
      correctOptionId: 'a',
      explanation:
        'Wo ... hin? is the colloquial split form of wohin?, asking about a destination.',
    },
    {
      id: 'ch53-ex-11',
      chapterNumber: 53,
      order: 11,
      type: 'singleChoice',
      level: 'controlled',
      grammarFocus: ['her', 'woher-split'],
      instruction: 'Choose the correct word to complete the colloquial split question.',
      prompt: 'Wo kommst du ___? – Ich komme aus der Schule.',
      options: [
        { id: 'a', text: 'her' },
        { id: 'b', text: 'hin' },
        { id: 'c', text: 'da' },
        { id: 'd', text: 'dort' },
      ],
      correctOptionId: 'a',
      explanation:
        'Wo ... her? is the colloquial split form of woher?, asking about an origin.',
    },
    {
      id: 'ch53-ex-12',
      chapterNumber: 53,
      order: 12,
      type: 'singleChoice',
      level: 'controlled',
      grammarFocus: ['her', 'speaker-perspective', 'dialogue'],
      instruction:
        "Read the exchange, then choose the adverb that fits the speaker's perspective.",
      prompt: 'Mama means: "Can you please come here?"',
      dialogue: [
        { speaker: 'Mama', german: 'Ich bin im Garten.', english: 'I am in the garden.' },
        {
          speaker: 'Mama',
          german: 'Kannst du bitte ___ kommen?',
        },
      ],
      options: [
        { id: 'a', text: 'her' },
        { id: 'b', text: 'hin' },
        { id: 'c', text: 'da' },
        { id: 'd', text: 'dort' },
      ],
      correctOptionId: 'a',
      explanation:
        'Mama is asking the child to move toward her own location, so her is correct.',
    },
    {
      id: 'ch53-ex-13',
      chapterNumber: 53,
      order: 13,
      type: 'singleChoice',
      level: 'controlled',
      grammarFocus: ['hin', 'speaker-perspective', 'dialogue'],
      instruction:
        "Read the exchange, then choose the adverb that fits the speaker's perspective.",
      prompt: 'Lisa means: "Okay, I will go there right away."',
      dialogue: [
        {
          speaker: 'Jonas',
          german: 'Kannst du zu Frau Klein gehen? Ich glaube, sie braucht Hilfe.',
          english: 'Can you go to Mrs. Klein? I think she needs help.',
        },
        {
          speaker: 'Lisa',
          german: 'Okay, ich gehe gleich ___.',
        },
      ],
      options: [
        { id: 'a', text: 'hin' },
        { id: 'b', text: 'her' },
        { id: 'c', text: 'da' },
        { id: 'd', text: 'dort' },
      ],
      correctOptionId: 'a',
      explanation:
        'Lisa is moving away from her own current position toward Frau Klein, so hin is correct.',
    },
    {
      id: 'ch53-ex-14',
      chapterNumber: 53,
      order: 14,
      type: 'singleChoice',
      level: 'controlled',
      grammarFocus: ['hierher', 'speaker-perspective', 'dialogue'],
      instruction:
        "Read the exchange, then choose the compound adverb that fits the speaker's perspective.",
      prompt: 'Anna means: "Do come here, we still have a free seat!"',
      dialogue: [
        {
          speaker: 'Anna',
          german: 'Ich sitze im Café am Marktplatz.',
          english: 'I am sitting in the café at the market square.',
        },
        {
          speaker: 'Anna',
          german: 'Komm doch ___, wir haben noch einen Platz frei!',
        },
      ],
      options: [
        { id: 'a', text: 'hierher' },
        { id: 'b', text: 'dorthin' },
        { id: 'c', text: 'daher' },
        { id: 'd', text: 'dahin' },
      ],
      correctOptionId: 'a',
      explanation:
        'Anna wants the listener to come to where she is, so the destination compound pointing toward the speaker, hierher, is correct.',
    },
    {
      id: 'ch53-ex-15',
      chapterNumber: 53,
      order: 15,
      type: 'singleChoice',
      level: 'transfer',
      grammarFocus: ['dahin', 'wohin', 'relative-clause', 'dialogue'],
      instruction:
        'Read the exchange, then choose the compound adverb that fits the answer.',
      prompt:
        'Schaffner means: "The train goes there, to where the trade fair is taking place."',
      dialogue: [
        {
          speaker: 'Paul',
          german: 'Wohin fährt der Zug um zehn Uhr?',
          english: "Where does the ten o'clock train go?",
        },
        {
          speaker: 'Schaffner',
          german: 'Der Zug fährt ___, wo die Messe stattfindet.',
        },
      ],
      options: [
        { id: 'a', text: 'dahin' },
        { id: 'b', text: 'daher' },
        { id: 'c', text: 'hierher' },
        { id: 'd', text: 'her' },
      ],
      correctOptionId: 'a',
      explanation:
        'The train\'s destination is away from the speaker (the conductor), so the hin-compound dahin fits, paired with a relative clause "wo ...".',
    },
    {
      id: 'ch53-ex-16',
      chapterNumber: 53,
      order: 16,
      type: 'singleChoice',
      level: 'transfer',
      grammarFocus: ['draussen', 'direction-vs-location', 'dialogue'],
      instruction:
        'Read the exchange, then choose the phrase that correctly expresses movement.',
      prompt: 'Opa means: "Just let him go outside!"',
      dialogue: [
        {
          speaker: 'Opa',
          german: 'Der Hund möchte nicht mehr drinnen bleiben.',
          english: 'The dog does not want to stay inside anymore.',
        },
        {
          speaker: 'Opa',
          german: 'Lass ihn doch ___ gehen!',
        },
      ],
      options: [
        { id: 'a', text: 'nach draußen' },
        { id: 'b', text: 'draußen' },
        { id: 'c', text: 'nach drinnen' },
        { id: 'd', text: 'drinnen' },
      ],
      correctOptionId: 'a',
      explanation:
        'draußen alone is a pure location adverb; expressing movement to the outside (wohin?) requires nach draußen.',
    },
    {
      id: 'ch53-ex-17',
      chapterNumber: 53,
      order: 17,
      type: 'textInput',
      level: 'recognition',
      grammarFocus: ['hier', 'location'],
      instruction: 'Fill in the missing adverb. Capitalisation is not checked.',
      prompt: 'Wo ist der Schlüssel? – Er liegt ___, direkt neben dir.',
      acceptedAnswers: ['hier'],
      answerMode: 'caseInsensitive',
      placeholder: 'hier',
      maxLength: 10,
      explanation: "Something within arm's reach of the speaker is described with hier.",
    },
    {
      id: 'ch53-ex-18',
      chapterNumber: 53,
      order: 18,
      type: 'textInput',
      level: 'recognition',
      grammarFocus: ['dort', 'location', 'distance'],
      instruction: 'Fill in the missing adverb. Capitalisation is not checked.',
      prompt: 'Schau mal, das Schloss ___ drüben ist über 500 Jahre alt.',
      acceptedAnswers: ['dort'],
      answerMode: 'caseInsensitive',
      placeholder: 'dort',
      maxLength: 10,
      explanation:
        'dort fits a place clearly separate from the speaker, here emphasized by drüben (over there).',
    },
    {
      id: 'ch53-ex-19',
      chapterNumber: 53,
      order: 19,
      type: 'textInput',
      level: 'recognition',
      grammarFocus: ['da', 'location'],
      instruction: 'Fill in the missing adverb. Capitalisation is not checked.',
      prompt: 'Es hat geklingelt. Ist ___ jemand?',
      acceptedAnswers: ['da'],
      answerMode: 'caseInsensitive',
      placeholder: 'da',
      maxLength: 10,
      explanation:
        'Ist da jemand? is the standard phrase for checking whether someone is present nearby.',
    },
    {
      id: 'ch53-ex-20',
      chapterNumber: 53,
      order: 20,
      type: 'textInput',
      level: 'controlled',
      grammarFocus: ['her', 'speaker-perspective', 'dialogue'],
      instruction:
        'Read the exchange and fill in the missing adverb. Capitalisation is not checked.',
      prompt: 'Tim means: "Come here, the game is about to start!"',
      dialogue: [
        {
          speaker: 'Tim',
          german: 'Ich bin gerade im Wohnzimmer.',
          english: 'I am currently in the living room.',
        },
        {
          speaker: 'Tim',
          german: 'Komm doch ___, das Spiel fängt gleich an!',
        },
      ],
      acceptedAnswers: ['her', 'hierher'],
      answerMode: 'caseInsensitive',
      placeholder: 'her',
      maxLength: 10,
      explanation:
        'Tim is inviting the listener to join him, so her (toward the speaker) is correct.',
    },
    {
      id: 'ch53-ex-21',
      chapterNumber: 53,
      order: 21,
      type: 'textInput',
      level: 'controlled',
      grammarFocus: ['hin', 'speaker-perspective', 'dialogue'],
      instruction:
        'Read the exchange and fill in the missing adverb. Capitalisation is not checked.',
      prompt: 'Frau Meier means: "Please go there quickly and get the package!"',
      dialogue: [
        {
          speaker: 'Frau Meier',
          german: 'Der Postbote wartet unten an der Tür.',
          english: 'The mailman is waiting downstairs at the door.',
        },
        {
          speaker: 'Frau Meier',
          german: 'Geh bitte schnell ___ und hol das Paket!',
        },
      ],
      acceptedAnswers: ['hin'],
      answerMode: 'caseInsensitive',
      placeholder: 'hin',
      maxLength: 10,
      explanation:
        'Frau Meier is sending the listener away from her own position, so hin is correct.',
    },
    {
      id: 'ch53-ex-22',
      chapterNumber: 53,
      order: 22,
      type: 'textInput',
      level: 'controlled',
      grammarFocus: ['hierher', 'compounds'],
      instruction: 'Fill in the missing compound adverb. Capitalisation is not checked.',
      prompt: 'Setz dich ___, neben mich!',
      acceptedAnswers: ['hierher'],
      answerMode: 'caseInsensitive',
      placeholder: 'hierher',
      maxLength: 15,
      explanation:
        "hierher (hier + her) expresses a destination that is the speaker's own location.",
    },
    {
      id: 'ch53-ex-23',
      chapterNumber: 53,
      order: 23,
      type: 'textInput',
      level: 'controlled',
      grammarFocus: ['dorthin', 'compounds'],
      instruction: 'Fill in the missing compound adverb. Capitalisation is not checked.',
      prompt:
        'Meine Freunde fahren an die Nordsee, und ich möchte auch gern einmal ___ fahren.',
      acceptedAnswers: ['dorthin', 'dahin'],
      answerMode: 'caseInsensitive',
      placeholder: 'dorthin',
      maxLength: 15,
      explanation: 'dorthin (dort + hin) expresses a destination away from the speaker.',
    },
    {
      id: 'ch53-ex-24',
      chapterNumber: 53,
      order: 24,
      type: 'textInput',
      level: 'controlled',
      grammarFocus: ['daher', 'compounds'],
      instruction: 'Fill in the missing compound adverb. Capitalisation is not checked.',
      prompt: 'Er kommt genau ___, wo die Musik spielt.',
      acceptedAnswers: ['daher'],
      answerMode: 'caseInsensitive',
      placeholder: 'daher',
      maxLength: 15,
      explanation:
        'daher (da + her) expresses an origin, with motion toward the speaker.',
    },
    {
      id: 'ch53-ex-25',
      chapterNumber: 53,
      order: 25,
      type: 'textInput',
      level: 'controlled',
      grammarFocus: ['dahin', 'compounds'],
      instruction: 'Fill in the missing compound adverb. Capitalisation is not checked.',
      prompt: 'Der Wanderweg führt genau ___, wo der Wasserfall ist.',
      acceptedAnswers: ['dahin', 'dorthin'],
      answerMode: 'caseInsensitive',
      placeholder: 'dahin',
      maxLength: 15,
      explanation: 'dahin (da + hin) expresses a destination away from the speaker.',
    },
    {
      id: 'ch53-ex-26',
      chapterNumber: 53,
      order: 26,
      type: 'textInput',
      level: 'production',
      grammarFocus: ['draussen', 'drinnen', 'location'],
      instruction: 'Fill in the missing adverb. Capitalisation is not checked.',
      prompt: 'Es ist kalt ___, aber im Haus ist es warm.',
      acceptedAnswers: ['draußen', 'draussen'],
      answerMode: 'caseInsensitive',
      placeholder: 'draußen',
      maxLength: 15,
      explanation:
        'draußen describes the location outside, contrasted here with the warm indoors.',
    },
    {
      id: 'ch53-ex-27',
      chapterNumber: 53,
      order: 27,
      type: 'textInput',
      level: 'production',
      grammarFocus: ['oben', 'unten', 'location'],
      instruction: 'Fill in the missing adverb. Capitalisation is not checked.',
      prompt: 'Der Keller ist ___, das Schlafzimmer ist im ersten Stock.',
      acceptedAnswers: ['unten'],
      answerMode: 'caseInsensitive',
      placeholder: 'unten',
      maxLength: 15,
      explanation: 'A cellar is normally below ground level, so unten fits.',
    },
    {
      id: 'ch53-ex-28',
      chapterNumber: 53,
      order: 28,
      type: 'textInput',
      level: 'production',
      grammarFocus: ['hin', 'wohin-split', 'dialogue'],
      instruction:
        'Read the exchange and fill in the missing word. Capitalisation is not checked.',
      prompt: 'Ben means: "So where are you going?"',
      dialogue: [
        {
          speaker: 'Nina',
          german: 'Ich muss gleich los.',
          english: 'I need to leave soon.',
        },
        {
          speaker: 'Ben',
          german: 'Wo gehst du denn ___?',
        },
      ],
      acceptedAnswers: ['hin'],
      answerMode: 'caseInsensitive',
      placeholder: 'hin',
      maxLength: 10,
      explanation:
        'Wo ... hin? is the colloquial split of wohin?, asking about a destination.',
    },
    {
      id: 'ch53-ex-29',
      chapterNumber: 53,
      order: 29,
      type: 'textInput',
      level: 'production',
      grammarFocus: ['her', 'woher-split', 'dialogue'],
      instruction:
        'Read the exchange and fill in the missing word. Capitalisation is not checked.',
      prompt: 'Chef means: "Where are you coming from right now?"',
      dialogue: [
        {
          speaker: 'Chef',
          german: 'Sie kommen aber spät zur Arbeit.',
          english: 'You are quite late to work.',
        },
        {
          speaker: 'Chef',
          german: 'Wo kommen Sie gerade ___?',
        },
      ],
      acceptedAnswers: ['her'],
      answerMode: 'caseInsensitive',
      placeholder: 'her',
      maxLength: 10,
      explanation:
        'Wo ... her? is the colloquial split of woher?, asking about an origin.',
    },
    {
      id: 'ch53-ex-30',
      chapterNumber: 53,
      order: 30,
      type: 'textInput',
      level: 'transfer',
      grammarFocus: ['error-correction', 'her', 'hin'],
      instruction:
        'Correct the mistake. Write the full, corrected sentence. Capitalisation and punctuation are checked.',
      prompt: 'Falsch: Komm hin zu mir!',
      acceptedAnswers: ['Komm her zu mir!'],
      answerMode: 'normalized',
      placeholder: 'Komm her zu mir!',
      maxLength: 40,
      explanation:
        'Asking someone to approach requires her (toward the speaker); hin would contradict the request by pointing away.',
    },
    {
      id: 'ch53-ex-31',
      chapterNumber: 53,
      order: 31,
      type: 'textInput',
      level: 'transfer',
      grammarFocus: ['sentence-production', 'her', 'compounds'],
      instruction:
        'Write a complete sentence using the cues. Capitalisation is checked; no full stop is required.',
      prompt: 'du (Befehl) – herkommen – sofort (bitte, dass jemand zu dir kommt)',
      acceptedAnswers: ['Komm sofort her!', 'Komm sofort her'],
      answerMode: 'normalized',
      placeholder: 'Komm sofort her!',
      maxLength: 40,
      explanation:
        'herkommen splits into her, moved to the end of the clause, when used as an imperative: Komm sofort her!',
    },
    {
      id: 'ch53-ex-32',
      chapterNumber: 53,
      order: 32,
      type: 'singleChoice',
      level: 'recognition',
      grammarFocus: ['vorne', 'hinten', 'location'],
      instruction: 'Choose the correct adverb.',
      prompt: 'Der Fahrer sitzt ganz ___ im Bus, die Kinder sitzen lieber hinten.',
      options: [
        { id: 'a', text: 'vorne' },
        { id: 'b', text: 'hinten' },
        { id: 'c', text: 'links' },
        { id: 'd', text: 'rechts' },
      ],
      correctOptionId: 'a',
      explanation:
        'vorne describes a position at the front, contrasted here with hinten (at the back).',
    },
    {
      id: 'ch53-ex-33',
      chapterNumber: 53,
      order: 33,
      type: 'singleChoice',
      level: 'recognition',
      grammarFocus: ['rechts', 'links', 'direction'],
      instruction: 'Choose the correct adverb.',
      prompt:
        'Um zum Bahnhof zu kommen, biegen Sie an der Ampel ___ ab, dann sehen Sie ihn schon. (turn right)',
      options: [
        { id: 'a', text: 'rechts' },
        { id: 'b', text: 'links' },
        { id: 'c', text: 'vorne' },
        { id: 'd', text: 'hinten' },
      ],
      correctOptionId: 'a',
      explanation:
        'rechts abbiegen means "to turn right", one of the two horizontal position adverbs used for directions.',
    },
    {
      id: 'ch53-ex-34',
      chapterNumber: 53,
      order: 34,
      type: 'singleChoice',
      level: 'controlled',
      grammarFocus: ['herunter', 'speaker-perspective', 'dialogue'],
      instruction:
        "Read the exchange, then choose the compound that fits the speaker's perspective.",
      prompt: 'Papa means: "Can you please bring me the book down here?"',
      dialogue: [
        {
          speaker: 'Papa',
          german: 'Ich bin schon unten in der Küche.',
          english: 'I am already downstairs in the kitchen.',
        },
        {
          speaker: 'Papa',
          german: 'Kannst du mir das Buch bitte ___ bringen?',
        },
      ],
      options: [
        { id: 'a', text: 'herunter' },
        { id: 'b', text: 'hinunter' },
        { id: 'c', text: 'herauf' },
        { id: 'd', text: 'hinauf' },
      ],
      correctOptionId: 'a',
      explanation:
        'Papa wants the book brought down to his own location, so the toward-the-speaker compound herunter is correct.',
    },
    {
      id: 'ch53-ex-35',
      chapterNumber: 53,
      order: 35,
      type: 'singleChoice',
      level: 'controlled',
      grammarFocus: ['herein', 'speaker-perspective', 'dialogue'],
      instruction:
        "Read the exchange, then choose the compound that fits the speaker's perspective.",
      prompt: 'Frau Bauer means: "Come in!"',
      dialogue: [
        {
          speaker: 'Frau Bauer',
          german: 'Es hat geklopft.',
          english: 'There was a knock at the door.',
        },
        { speaker: 'Frau Bauer', german: 'Kommen Sie ___!' },
      ],
      options: [
        { id: 'a', text: 'herein' },
        { id: 'b', text: 'hinein' },
        { id: 'c', text: 'heraus' },
        { id: 'd', text: 'hinaus' },
      ],
      correctOptionId: 'a',
      explanation:
        'Frau Bauer, standing inside, invites the visitor toward her own location, so herein (toward the speaker) is correct.',
    },
    {
      id: 'ch53-ex-36',
      chapterNumber: 53,
      order: 36,
      type: 'singleChoice',
      level: 'controlled',
      grammarFocus: ['hinaus', 'speaker-perspective', 'dialogue'],
      instruction:
        "Read the exchange, then choose the compound that fits the speaker's perspective.",
      prompt: 'Frau Klein means: "Go out, tomcat!"',
      dialogue: [
        {
          speaker: 'Frau Klein',
          german: 'Die Katze will unbedingt nach draußen.',
          english: 'The cat absolutely wants to go outside.',
        },
        {
          speaker: 'Frau Klein',
          german: 'Geh ___, du Kater!',
        },
      ],
      options: [
        { id: 'a', text: 'hinaus' },
        { id: 'b', text: 'heraus' },
        { id: 'c', text: 'hinein' },
        { id: 'd', text: 'herein' },
      ],
      correctOptionId: 'a',
      explanation:
        'Frau Klein is inside and sends the cat away from her own location, so hinaus (away from the speaker) is correct.',
    },
    {
      id: 'ch53-ex-37',
      chapterNumber: 53,
      order: 37,
      type: 'singleChoice',
      level: 'recognition',
      grammarFocus: ['vorne', 'hinten', 'location'],
      instruction: 'Choose the correct adverb.',
      prompt: 'Im Kino sitzen wir lieber ___, damit wir alles gut sehen können.',
      options: [
        { id: 'a', text: 'vorne' },
        { id: 'b', text: 'hinten' },
        { id: 'c', text: 'oben' },
        { id: 'd', text: 'unten' },
      ],
      correctOptionId: 'a',
      explanation: 'vorne describes a seat at the front of the cinema.',
    },
    {
      id: 'ch53-ex-38',
      chapterNumber: 53,
      order: 38,
      type: 'singleChoice',
      level: 'transfer',
      grammarFocus: ['links', 'rechts', 'direction', 'dialogue'],
      instruction: 'Read the exchange, then choose the correct adverb.',
      prompt:
        'Passantin means: "Go left here, then you will already see the post office."',
      dialogue: [
        {
          speaker: 'Tourist',
          german: 'Entschuldigung, wo ist die Post?',
          english: 'Excuse me, where is the post office?',
        },
        {
          speaker: 'Passantin',
          german: 'Gehen Sie hier ___, dann sehen Sie die Post schon.',
        },
      ],
      options: [
        { id: 'a', text: 'links' },
        { id: 'b', text: 'rechts' },
        { id: 'c', text: 'vorne' },
        { id: 'd', text: 'hinten' },
      ],
      correctOptionId: 'a',
      explanation:
        'links gives a direction to the left, one of the two horizontal position adverbs used in directions.',
    },
    {
      id: 'ch53-ex-39',
      chapterNumber: 53,
      order: 39,
      type: 'singleChoice',
      level: 'transfer',
      grammarFocus: ['hinauf', 'direction'],
      instruction: 'Choose the correct compound adverb.',
      prompt: 'Die Bergsteiger mussten den steilen Pfad mühsam ___ klettern.',
      options: [
        { id: 'a', text: 'hinauf' },
        { id: 'b', text: 'herauf' },
        { id: 'c', text: 'hinunter' },
        { id: 'd', text: 'herunter' },
      ],
      correctOptionId: 'a',
      explanation:
        'Describing the climbers\' own upward movement away from the base of the mountain, without a "toward the speaker" perspective, uses hinauf.',
    },
    {
      id: 'ch53-ex-40',
      chapterNumber: 53,
      order: 40,
      type: 'singleChoice',
      level: 'recognition',
      grammarFocus: ['herkommen', 'meaning'],
      instruction: 'Choose the correct meaning.',
      prompt: 'Was bedeutet "herkommen"?',
      options: [
        { id: 'a', text: 'to come here (toward the speaker)' },
        { id: 'b', text: 'to go there (away from the speaker)' },
        { id: 'c', text: 'to stay inside' },
        { id: 'd', text: 'to stay outside' },
      ],
      correctOptionId: 'a',
      explanation:
        'herkommen combines kommen with her, so it means moving toward the speaker.',
    },
    {
      id: 'ch53-ex-41',
      chapterNumber: 53,
      order: 41,
      type: 'singleChoice',
      level: 'transfer',
      grammarFocus: ['hinunter', 'direction'],
      instruction: 'Choose the correct compound adverb.',
      prompt: 'Nach dem Gipfel mussten wir den Berg wieder ___ gehen, zurück zum Auto.',
      options: [
        { id: 'a', text: 'hinunter' },
        { id: 'b', text: 'herunter' },
        { id: 'c', text: 'hinauf' },
        { id: 'd', text: 'herauf' },
      ],
      correctOptionId: 'a',
      explanation:
        'Describing the group\'s own downward movement away from the summit, without a "toward the speaker" perspective, uses hinunter.',
    },
    {
      id: 'ch53-ex-42',
      chapterNumber: 53,
      order: 42,
      type: 'textInput',
      level: 'recognition',
      grammarFocus: ['vorne', 'location'],
      instruction: 'Fill in the missing adverb. Capitalisation is not checked.',
      prompt: 'Der Busfahrer sitzt ___, die Fahrgäste steigen hinten ein.',
      acceptedAnswers: ['vorne', 'vorn'],
      answerMode: 'caseInsensitive',
      placeholder: 'vorne',
      maxLength: 10,
      explanation:
        'vorne describes the front position, contrasted with hinten (at the back).',
    },
    {
      id: 'ch53-ex-43',
      chapterNumber: 53,
      order: 43,
      type: 'textInput',
      level: 'recognition',
      grammarFocus: ['hinten', 'location'],
      instruction: 'Fill in the missing adverb. Capitalisation is not checked.',
      prompt: 'Der Kofferraum ist ___, dort verstauen wir die Koffer.',
      acceptedAnswers: ['hinten'],
      answerMode: 'caseInsensitive',
      placeholder: 'hinten',
      maxLength: 10,
      explanation: 'hinten describes the back of the car, where the trunk is located.',
    },
    {
      id: 'ch53-ex-44',
      chapterNumber: 53,
      order: 44,
      type: 'textInput',
      level: 'recognition',
      grammarFocus: ['links', 'direction'],
      instruction: 'Fill in the missing adverb. Capitalisation is not checked.',
      prompt: 'Die Apotheke finden Sie ___ um die Ecke, direkt gegenüber der Kirche. (on the left)',
      acceptedAnswers: ['links'],
      answerMode: 'caseInsensitive',
      placeholder: 'links',
      maxLength: 10,
      explanation: 'links gives a direction to the left.',
    },
    {
      id: 'ch53-ex-45',
      chapterNumber: 53,
      order: 45,
      type: 'textInput',
      level: 'controlled',
      grammarFocus: ['rechts', 'direction'],
      instruction: 'Fill in the missing adverb. Capitalisation is not checked.',
      prompt: 'Biegen Sie an der nächsten Kreuzung ___ ab, dann sehen Sie das Museum. (turn right)',
      acceptedAnswers: ['rechts'],
      answerMode: 'caseInsensitive',
      placeholder: 'rechts',
      maxLength: 10,
      explanation: 'rechts gives a direction to the right.',
    },
    {
      id: 'ch53-ex-46',
      chapterNumber: 53,
      order: 46,
      type: 'textInput',
      level: 'controlled',
      grammarFocus: ['herunter', 'speaker-perspective', 'dialogue'],
      instruction:
        'Read the exchange and fill in the missing word. Capitalisation is not checked.',
      prompt: 'Mama means: "Can you please bring the towel down here?"',
      dialogue: [
        {
          speaker: 'Mama',
          german: 'Ich bin schon unten am Pool.',
          english: 'I am already downstairs at the pool.',
        },
        {
          speaker: 'Mama',
          german: 'Kannst du das Handtuch bitte ___ bringen?',
        },
      ],
      acceptedAnswers: ['herunter'],
      answerMode: 'caseInsensitive',
      placeholder: 'herunter',
      maxLength: 15,
      explanation:
        'Mama wants the towel brought down to her own location, so herunter (toward the speaker) is correct.',
    },
    {
      id: 'ch53-ex-47',
      chapterNumber: 53,
      order: 47,
      type: 'textInput',
      level: 'controlled',
      grammarFocus: ['hinunter', 'speaker-perspective', 'dialogue'],
      instruction:
        'Read the exchange and fill in the missing word. Capitalisation is not checked.',
      prompt: 'Sara means: "Go on down, I\'ll wait up here."',
      dialogue: [
        {
          speaker: 'Lukas',
          german: 'Ich muss noch schnell in den Keller.',
          english: 'I still need to quickly go to the cellar.',
        },
        {
          speaker: 'Sara',
          german: 'Geh ruhig ___, ich warte hier oben.',
        },
      ],
      acceptedAnswers: ['hinunter'],
      answerMode: 'caseInsensitive',
      placeholder: 'hinunter',
      maxLength: 15,
      explanation:
        'Sara stays upstairs while Lukas moves away from her, downward, so hinunter is correct.',
    },
    {
      id: 'ch53-ex-48',
      chapterNumber: 53,
      order: 48,
      type: 'textInput',
      level: 'controlled',
      grammarFocus: ['herauf', 'speaker-perspective', 'dialogue'],
      instruction:
        'Read the exchange and fill in the missing word. Capitalisation is not checked.',
      prompt: 'Emma means: "Do come up, there is room for everyone!"',
      dialogue: [
        {
          speaker: 'Emma',
          german: 'Wir sitzen hier oben auf der Dachterrasse.',
          english: 'We are sitting up here on the roof terrace.',
        },
        {
          speaker: 'Emma',
          german: 'Kommt doch ___, hier ist Platz für alle!',
        },
      ],
      acceptedAnswers: ['herauf'],
      answerMode: 'caseInsensitive',
      placeholder: 'herauf',
      maxLength: 15,
      explanation:
        'Emma invites the others toward her own location, upward, so herauf (toward the speaker) is correct.',
    },
    {
      id: 'ch53-ex-49',
      chapterNumber: 53,
      order: 49,
      type: 'textInput',
      level: 'production',
      grammarFocus: ['hinauf', 'direction', 'dialogue'],
      instruction:
        'Read the exchange and fill in the missing word. Capitalisation is not checked.',
      prompt: 'Ben means: "Then we will just have to walk up on foot."',
      dialogue: [
        {
          speaker: 'Ben',
          german: 'Der Aufzug ist leider kaputt.',
          english: 'The elevator is unfortunately broken.',
        },
        {
          speaker: 'Ben',
          german: 'Dann müssen wir eben zu Fuß ___ gehen.',
        },
      ],
      acceptedAnswers: ['hinauf'],
      answerMode: 'caseInsensitive',
      placeholder: 'hinauf',
      maxLength: 15,
      explanation:
        "Describing the group's own upward movement, without a toward-the-speaker perspective, uses hinauf.",
    },
    {
      id: 'ch53-ex-50',
      chapterNumber: 53,
      order: 50,
      type: 'textInput',
      level: 'production',
      grammarFocus: ['herein', 'speaker-perspective', 'dialogue'],
      instruction:
        'Read the exchange and fill in the missing word. Capitalisation is not checked.',
      prompt: 'Herr Fischer means: "Come in!"',
      dialogue: [
        {
          speaker: 'Herr Fischer',
          german: 'Es hat geklopft, mach bitte auf!',
          english: 'There was a knock, please open the door!',
        },
        { speaker: 'Herr Fischer', german: 'Kommen Sie ___!' },
      ],
      acceptedAnswers: ['herein'],
      answerMode: 'caseInsensitive',
      placeholder: 'herein',
      maxLength: 15,
      explanation:
        'Herr Fischer, standing inside, invites the visitor toward his own location, so herein is correct.',
    },
    {
      id: 'ch53-ex-51',
      chapterNumber: 53,
      order: 51,
      type: 'textInput',
      level: 'production',
      grammarFocus: ['hinein', 'direction', 'dialogue'],
      instruction:
        'Read the exchange and fill in the missing word. Capitalisation is not checked.',
      prompt: 'Nora means: "Go on in, I will follow in a moment."',
      dialogue: [
        { speaker: 'Nora', german: 'Die Tür steht offen.', english: 'The door is open.' },
        {
          speaker: 'Nora',
          german: 'Geh ruhig schon ___, ich komme gleich nach.',
        },
      ],
      acceptedAnswers: ['hinein'],
      answerMode: 'caseInsensitive',
      placeholder: 'hinein',
      maxLength: 15,
      explanation:
        'Nora sends the listener ahead of her, away from her own current position, so hinein is correct.',
    },
    {
      id: 'ch53-ex-52',
      chapterNumber: 53,
      order: 52,
      type: 'textInput',
      level: 'production',
      grammarFocus: ['heraus', 'speaker-perspective', 'dialogue'],
      instruction:
        'Read the exchange and fill in the missing word. Capitalisation is not checked.',
      prompt: 'Opa means: "Please come out, we are already waiting for you!"',
      dialogue: [
        {
          speaker: 'Opa',
          german: 'Das Essen ist fertig!',
          english: 'The food is ready!',
        },
        {
          speaker: 'Opa',
          german: 'Komm bitte ___, wir warten schon auf dich!',
        },
      ],
      acceptedAnswers: ['heraus'],
      answerMode: 'caseInsensitive',
      placeholder: 'heraus',
      maxLength: 15,
      explanation:
        'Opa is calling the listener toward the group he is part of, so heraus (toward the speaker) is correct.',
    },
    {
      id: 'ch53-ex-53',
      chapterNumber: 53,
      order: 53,
      type: 'textInput',
      level: 'production',
      grammarFocus: ['hinaus', 'direction', 'dialogue'],
      instruction:
        'Read the exchange and fill in the missing word. Capitalisation is not checked.',
      prompt: 'Frau Weber means: "Just let him out into the garden!"',
      dialogue: [
        {
          speaker: 'Frau Weber',
          german: 'Der Hund bellt schon die ganze Zeit.',
          english: 'The dog has been barking the whole time.',
        },
        {
          speaker: 'Frau Weber',
          german: 'Lass ihn doch ___ in den Garten!',
        },
      ],
      acceptedAnswers: ['hinaus'],
      answerMode: 'caseInsensitive',
      placeholder: 'hinaus',
      maxLength: 15,
      explanation:
        'Frau Weber sends the dog away from her own location into the garden, so hinaus is correct.',
    },
    {
      id: 'ch53-ex-54',
      chapterNumber: 53,
      order: 54,
      type: 'textInput',
      level: 'transfer',
      grammarFocus: ['error-correction', 'her', 'hin'],
      instruction:
        'Correct the mistake. Write the full, corrected sentence. Capitalisation and punctuation are checked.',
      prompt: 'Falsch: Geh her und hol das Paket!',
      acceptedAnswers: ['Geh hin und hol das Paket!'],
      answerMode: 'normalized',
      placeholder: 'Geh hin und hol das Paket!',
      maxLength: 40,
      explanation:
        "Sending someone away from the speaker's own location requires hin; her would mean toward the speaker, the opposite meaning.",
    },
    {
      id: 'ch53-ex-55',
      chapterNumber: 53,
      order: 55,
      type: 'textInput',
      level: 'transfer',
      grammarFocus: ['sentence-production', 'herein', 'compounds'],
      instruction:
        'Write a complete sentence using the cues. Capitalisation is checked; no full stop is required.',
      prompt:
        'du (Bitte) – hereinkommen – sofort (du bittest jemanden, ins Zimmer zu kommen, in dem du selbst stehst)',
      acceptedAnswers: ['Komm sofort herein!', 'Komm sofort herein'],
      answerMode: 'normalized',
      placeholder: 'Komm sofort herein!',
      maxLength: 40,
      explanation:
        "hereinkommen splits into herein, moved to the end of the clause, when used as an imperative inviting someone toward the speaker's own room: Komm sofort herein!",
    },
    {
      id: 'ch53-ex-56',
      chapterNumber: 53,
      order: 56,
      type: 'matching',
      level: 'recognition',
      grammarFocus: ['opposites', 'location'],
      instruction: 'Match each position adverb with its opposite.',
      prompt: 'Match the adverb pairs.',
      pairs: [
        { id: 'p1', left: 'oben', right: 'unten' },
        { id: 'p2', left: 'drinnen', right: 'draußen' },
        { id: 'p3', left: 'vorne', right: 'hinten' },
        { id: 'p4', left: 'links', right: 'rechts' },
      ],
      explanation:
        'oben/unten, drinnen/draußen, vorne/hinten, and links/rechts are the core opposite pairs among the pure location adverbs.',
    },
    {
      id: 'ch53-ex-57',
      chapterNumber: 53,
      order: 57,
      type: 'matching',
      level: 'controlled',
      grammarFocus: ['hin', 'her', 'compounds', 'opposites'],
      instruction:
        'Match each her-compound with its hin-counterpart, pointing the opposite direction.',
      prompt: 'Match the direction pairs.',
      pairs: [
        { id: 'p1', left: 'herein', right: 'hinein' },
        { id: 'p2', left: 'herauf', right: 'hinauf' },
        { id: 'p3', left: 'herunter', right: 'hinunter' },
        { id: 'p4', left: 'heraus', right: 'hinaus' },
      ],
      explanation:
        'Each pair shares the same location prefix (ein-, auf-, unter-, aus-) but differs in direction: her- toward the speaker, hin- away from the speaker.',
    },
    {
      id: 'ch53-ex-58',
      chapterNumber: 53,
      order: 58,
      type: 'matching',
      level: 'recognition',
      grammarFocus: ['hier', 'da', 'dort', 'oben', 'meaning'],
      instruction: 'Match each adverb with its English meaning.',
      prompt: 'Match the adverbs with their meanings.',
      pairs: [
        { id: 'p1', left: 'hier', right: 'here' },
        { id: 'p2', left: 'dort', right: 'there (farther away)' },
        { id: 'p3', left: 'da', right: 'there (nearby)' },
        { id: 'p4', left: 'oben', right: 'up / above' },
        { id: 'p5', left: 'drinnen', right: 'inside' },
      ],
      explanation:
        'hier, dort, and da all mean roughly "here/there" but differ in distance from the speaker; oben and drinnen name vertical position and interior location.',
    },
    {
      id: 'ch53-ex-59',
      chapterNumber: 53,
      order: 59,
      type: 'matching',
      level: 'transfer',
      grammarFocus: ['separable-verbs', 'hin', 'her', 'meaning'],
      instruction: 'Match each separable verb with its English meaning.',
      prompt: 'Match the verbs with their meanings.',
      pairs: [
        { id: 'p1', left: 'hingehen', right: 'to go there' },
        { id: 'p2', left: 'herkommen', right: 'to come here' },
        { id: 'p3', left: 'hineingehen', right: 'to go inside' },
        { id: 'p4', left: 'herauskommen', right: 'to come out' },
        { id: 'p5', left: 'hinaufsteigen', right: 'to climb up' },
      ],
      explanation:
        'hin- and her- attach to verbs of motion as separable prefixes, adding a directional meaning relative to the speaker.',
    },
    {
      id: 'ch53-ex-60',
      chapterNumber: 53,
      order: 60,
      type: 'matching',
      level: 'controlled',
      grammarFocus: ['compounds', 'word-formation'],
      instruction: 'Match each compound with the two words it is built from.',
      prompt: 'Match the compounds with their components.',
      pairs: [
        { id: 'p1', left: 'hierher', right: 'hier + her' },
        { id: 'p2', left: 'dorthin', right: 'dort + hin' },
        { id: 'p3', left: 'daher', right: 'da + her' },
        { id: 'p4', left: 'dahin', right: 'da + hin' },
      ],
      explanation:
        'The second half of each compound (-her or -hin) always signals the direction relative to the speaker.',
    },
    {
      id: 'ch53-ex-61',
      chapterNumber: 53,
      order: 61,
      type: 'matching',
      level: 'controlled',
      grammarFocus: ['nach', 'location-vs-destination'],
      instruction:
        'Match each pure location adverb with the destination phrase built from it.',
      prompt: 'Match the location adverbs with their destination form.',
      pairs: [
        { id: 'p1', left: 'oben', right: 'nach oben' },
        { id: 'p2', left: 'unten', right: 'nach unten' },
        { id: 'p3', left: 'draußen', right: 'nach draußen' },
        { id: 'p4', left: 'drinnen', right: 'nach drinnen' },
      ],
      explanation:
        'oben, unten, draußen, and drinnen answer wo? by themselves; to express a destination (wohin?), German adds nach.',
    },
    {
      id: 'ch53-ex-62',
      chapterNumber: 53,
      order: 62,
      type: 'matching',
      level: 'transfer',
      grammarFocus: ['hin', 'her', 'speaker-perspective', 'context'],
      instruction: 'Match each situation with the adverb that correctly completes it.',
      prompt: 'Match the situations with the correct adverb or compound.',
      pairs: [
        { id: 'p1', left: 'Die Mutter ruft das Kind zu sich.', right: 'her' },
        { id: 'p2', left: 'Der Vater schickt das Kind zum Laden.', right: 'hin' },
        {
          id: 'p3',
          left: 'Die Gastgeberin bittet die Gäste, ins Haus zu kommen.',
          right: 'herein',
        },
        { id: 'p4', left: 'Der Ball fliegt aus dem Fenster.', right: 'hinaus' },
      ],
      explanation:
        "The direction always depends on the speaker's own position: her/herein for motion toward them, hin/hinaus for motion away from them.",
    },
    {
      id: 'ch53-ex-63',
      chapterNumber: 53,
      order: 63,
      type: 'matching',
      level: 'production',
      grammarFocus: ['vorne', 'hinten', 'links', 'rechts', 'context'],
      instruction: 'Match each adverb with an example sentence that uses it correctly.',
      prompt: 'Match the adverbs with their examples.',
      pairs: [
        { id: 'p1', left: 'vorne', right: 'Er sitzt vorne im Bus.' },
        { id: 'p2', left: 'hinten', right: 'Der Koffer liegt hinten im Kofferraum.' },
        { id: 'p3', left: 'links', right: 'Die Post ist links um die Ecke.' },
        { id: 'p4', left: 'rechts', right: 'Bieg an der Kreuzung rechts ab.' },
      ],
      explanation:
        'vorne/hinten and links/rechts locate something along the front-back and side-to-side axes.',
    },
    {
      id: 'ch53-ex-64',
      chapterNumber: 53,
      order: 64,
      type: 'dragToSlots',
      level: 'recognition',
      grammarFocus: ['draußen', 'drinnen', 'location'],
      instruction: 'Drag the correct word into the slot.',
      prompt: 'Die Kinder spielen ___ im Garten, weil die Sonne scheint.',
      templateParts: ['Die Kinder spielen ', ' im Garten, weil die Sonne scheint.'],
      slots: [{ id: 's1', correctWord: 'draußen' }],
      wordBank: ['draußen', 'drinnen', 'oben', 'unten'],
      explanation: 'Playing in the garden in the sunshine happens draußen (outside).',
    },
    {
      id: 'ch53-ex-65',
      chapterNumber: 53,
      order: 65,
      type: 'dragToSlots',
      level: 'controlled',
      grammarFocus: ['her', 'speaker-perspective'],
      instruction: 'Drag the correct word into the slot.',
      prompt: 'Komm bitte sofort ___, ich brauche dich!',
      templateParts: ['Komm bitte sofort ', ', ich brauche dich!'],
      slots: [{ id: 's1', correctWord: 'her' }],
      wordBank: ['her', 'hin', 'dort', 'da'],
      explanation: 'Asking someone to approach requires her (toward the speaker).',
    },
    {
      id: 'ch53-ex-66',
      chapterNumber: 53,
      order: 66,
      type: 'dragToSlots',
      level: 'controlled',
      grammarFocus: ['hin', 'speaker-perspective'],
      instruction: 'Drag the correct word into the slot.',
      prompt: 'Geh ___ und hol die Post vom Briefkasten!',
      templateParts: ['Geh ', ' und hol die Post vom Briefkasten!'],
      slots: [{ id: 's1', correctWord: 'hin' }],
      wordBank: ['hin', 'her', 'da', 'dort'],
      explanation: "Sending someone away from the speaker's own location requires hin.",
    },
    {
      id: 'ch53-ex-67',
      chapterNumber: 53,
      order: 67,
      type: 'dragToSlots',
      level: 'transfer',
      grammarFocus: ['hinauf', 'herauf', 'speaker-perspective'],
      instruction: 'Drag the correct word into the slot.',
      prompt: 'Steig bitte die Treppe ___, dein Zimmer ist im ersten Stock.',
      templateParts: ['Steig bitte die Treppe ', ', dein Zimmer ist im ersten Stock.'],
      slots: [{ id: 's1', correctWord: 'hinauf' }],
      wordBank: ['hinauf', 'herauf', 'hinunter', 'herunter'],
      explanation:
        "Describing the listener's own upward movement, without a toward-the-speaker perspective, uses hinauf.",
    },
    {
      id: 'ch53-ex-68',
      chapterNumber: 53,
      order: 68,
      type: 'dragToSlots',
      level: 'transfer',
      grammarFocus: ['herauf', 'hinauf', 'speaker-perspective'],
      instruction: 'Drag the correct word into the slot.',
      prompt:
        'Der Kellner kommt mit dem Tablett die Treppe ___, um uns oben zu bedienen.',
      templateParts: [
        'Der Kellner kommt mit dem Tablett die Treppe ',
        ', um uns oben zu bedienen.',
      ],
      slots: [{ id: 's1', correctWord: 'herauf' }],
      wordBank: ['herauf', 'hinauf', 'herunter', 'hinunter'],
      explanation:
        "The waiter is moving toward the speaker's own location upstairs, so herauf (toward the speaker) is correct.",
    },
    {
      id: 'ch53-ex-69',
      chapterNumber: 53,
      order: 69,
      type: 'dragToSlots',
      level: 'transfer',
      grammarFocus: ['hinunter', 'herunter', 'speaker-perspective'],
      instruction: 'Drag the correct word into the slot.',
      prompt: 'Lauf schnell die Treppe ___, der Bus wartet nicht!',
      templateParts: ['Lauf schnell die Treppe ', ', der Bus wartet nicht!'],
      slots: [{ id: 's1', correctWord: 'hinunter' }],
      wordBank: ['hinunter', 'herunter', 'hinauf', 'herauf'],
      explanation:
        "Sending the listener away from the speaker's current position, downward, uses hinunter.",
    },
    {
      id: 'ch53-ex-70',
      chapterNumber: 53,
      order: 70,
      type: 'dragToSlots',
      level: 'transfer',
      grammarFocus: ['herunter', 'hinunter', 'speaker-perspective'],
      instruction: 'Drag the correct word into the slot.',
      prompt: 'Wenn du fertig bist, komm bitte die Treppe ___ zu mir in die Küche.',
      templateParts: [
        'Wenn du fertig bist, komm bitte die Treppe ',
        ' zu mir in die Küche.',
      ],
      slots: [{ id: 's1', correctWord: 'herunter' }],
      wordBank: ['herunter', 'hinunter', 'herauf', 'hinauf'],
      explanation:
        "Coming to the speaker's own location downstairs uses herunter (toward the speaker).",
    },
    {
      id: 'ch53-ex-71',
      chapterNumber: 53,
      order: 71,
      type: 'dragToSlots',
      level: 'production',
      grammarFocus: ['links', 'rechts', 'vorne', 'hinten', 'direction'],
      instruction: 'Drag the correct word into the slot.',
      prompt: 'Der Supermarkt ist nicht weit, er liegt gleich ___ um die Ecke. (on the left)',
      templateParts: ['Der Supermarkt ist nicht weit, er liegt gleich ', ' um die Ecke.'],
      slots: [{ id: 's1', correctWord: 'links' }],
      wordBank: ['links', 'rechts', 'vorne', 'hinten'],
      explanation:
        'links describes a position or direction to the left, here around the corner.',
    },
    {
      id: 'ch53-ex-72',
      chapterNumber: 53,
      order: 72,
      type: 'dragToSlots',
      level: 'transfer',
      grammarFocus: ['hin', 'her', 'contrast'],
      instruction: 'Drag the correct words into the slots.',
      prompt: 'Sie geht ___, und du kommst bitte ___.',
      templateParts: ['Sie geht ', ', und du kommst bitte ', '.'],
      slots: [
        { id: 's1', correctWord: 'hin' },
        { id: 's2', correctWord: 'her' },
      ],
      wordBank: ['hin', 'her', 'da', 'dort'],
      explanation:
        'She moves away from the speaker (hin), while the listener is asked to move toward the speaker (her).',
    },
    {
      id: 'ch53-ex-73',
      chapterNumber: 53,
      order: 73,
      type: 'errorSpotting',
      level: 'controlled',
      grammarFocus: ['hin', 'her', 'error-correction'],
      instruction: 'Find the word that does not fit the meaning of the sentence.',
      prompt:
        'The speaker is asking the listener to approach, but one word points the wrong way.',
      tokens: ['Komm', 'hin', 'zu', 'mir!'],
      errorTokenIndex: 1,
      correction: 'her',
      explanation:
        'Asking someone to approach requires her (toward the speaker); hin would mean moving away, contradicting "zu mir".',
    },
    {
      id: 'ch53-ex-74',
      chapterNumber: 53,
      order: 74,
      type: 'errorSpotting',
      level: 'controlled',
      grammarFocus: ['hin', 'her', 'error-correction'],
      instruction: 'Find the word that does not fit the meaning of the sentence.',
      prompt:
        'The speaker is sending the listener away to ask someone else, but one word points the wrong way.',
      tokens: ['Geh', 'her', 'und', 'frag', 'ihn!'],
      errorTokenIndex: 1,
      correction: 'hin',
      explanation:
        'Sending someone away from the speaker requires hin; her would mean toward the speaker, the opposite meaning.',
    },
    {
      id: 'ch53-ex-75',
      chapterNumber: 53,
      order: 75,
      type: 'errorSpotting',
      level: 'transfer',
      grammarFocus: ['hinein', 'herein', 'error-correction'],
      instruction: "Find the word that does not fit the speaker's perspective.",
      prompt:
        'The speaker is standing inside the house, welcoming guests toward their own location.',
      tokens: ['Die', 'Gäste', 'kommen', 'hinein', 'in', 'das', 'Haus.'],
      errorTokenIndex: 3,
      correction: 'herein',
      explanation:
        'Guests moving toward the speaker, who is already inside, requires herein, not hinein (which would mean away from the speaker).',
    },
    {
      id: 'ch53-ex-76',
      chapterNumber: 53,
      order: 76,
      type: 'errorSpotting',
      level: 'transfer',
      grammarFocus: ['hinein', 'herein', 'error-correction'],
      instruction: "Find the word that does not fit the speaker's perspective.",
      prompt:
        'The speaker describes their own movement away, into the cellar, with no one waiting there.',
      tokens: ['Ich', 'gehe', 'jetzt', 'herein', 'in', 'den', 'Keller.'],
      errorTokenIndex: 3,
      correction: 'hinein',
      explanation:
        "The speaker's own movement into the cellar, away from wherever they currently are, requires hinein, not herein (which implies toward someone else).",
    },
    {
      id: 'ch53-ex-77',
      chapterNumber: 53,
      order: 77,
      type: 'errorSpotting',
      level: 'transfer',
      grammarFocus: ['dahin', 'hierher', 'error-correction'],
      instruction: 'Find the word that does not fit the invitation.',
      prompt: 'The speaker is inviting the listener to join them at their own table.',
      tokens: ['Komm', 'doch', 'dahin,', 'wir', 'sitzen', 'hier!'],
      errorTokenIndex: 2,
      correction: 'hierher,',
      explanation:
        'dahin points away from the speaker, but the invitation is to join the speaker at "hier"; the toward-the-speaker compound hierher is needed instead.',
    },
    {
      id: 'ch53-ex-78',
      chapterNumber: 53,
      order: 78,
      type: 'errorSpotting',
      level: 'transfer',
      grammarFocus: ['draußen', 'nach draußen', 'error-correction'],
      instruction: 'Find the word that cannot express movement by itself.',
      prompt:
        'The speaker wants the dog to be allowed to go outside, not merely to be located outside.',
      tokens: ['Lass', 'ihn', 'draußen', 'gehen!'],
      errorTokenIndex: 2,
      correction: 'nach draußen',
      explanation:
        'draußen is a pure location adverb (wo?); expressing movement to the outside (wohin?) requires nach draußen.',
    },
    {
      id: 'ch53-ex-79',
      chapterNumber: 53,
      order: 79,
      type: 'errorSpotting',
      level: 'transfer',
      grammarFocus: ['hin', 'her', 'wohin-split', 'error-correction'],
      instruction: 'Find the word that does not match the intended question.',
      prompt:
        'The speaker wants to ask where the listener is going (a destination), not where they are coming from.',
      tokens: ['Wo', 'gehst', 'du', 'her?'],
      errorTokenIndex: 3,
      correction: 'hin?',
      explanation:
        'her belongs to woher? (origin); asking about a destination with gehen requires the split form of wohin?, ending in hin.',
    },
    {
      id: 'ch53-ex-80',
      chapterNumber: 53,
      order: 80,
      type: 'errorSpotting',
      level: 'transfer',
      grammarFocus: ['oben', 'unten', 'error-correction'],
      instruction:
        'Find the word that contradicts common knowledge about where a cellar is.',
      prompt: 'A cellar is normally the lowest room in a house, below the attic.',
      tokens: [
        'Der',
        'Keller',
        'ist',
        'oben,',
        'und',
        'das',
        'Dachgeschoss',
        'ist',
        'ganz',
        'oben.',
      ],
      errorTokenIndex: 3,
      correction: 'unten,',
      explanation:
        "A cellar (Keller) is normally below ground level, so unten is correct, not oben, which contradicts the sentence's own comparison with the attic.",
    },
  ],
};
