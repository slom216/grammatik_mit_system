    ...
    <div className="stack">
      <header>
        ...
        <p className="text-muted prose">
          How many chapters you have completed each day. Everything here is stored in
          this browser only.
        </p>
      </header>
      ...
      <Card title="Chapters completed" titleLevel={2}>
        <div className="stack">
          <p className="text-sm text-muted">
            {summary.totalCompleted} {summary.totalCompleted === 1 ? 'chapter' : 'chapters'}{' '}
            completed across {summary.activeDays}{' '}
            {summary.activeDays === 1 ? 'day' : 'days'}
            {summary.bestDay
              ? ` · best day: ${summary.bestDay.count} on ${formatDate(summary.bestDay.date)}`
              : ''}
            .
          </p>
          <ActivityCalendar byDay={byDay} />
          <StreakDisplay histories={histories} />
        </div>
      </Card>

      <Card title="Day by day" titleLevel={2}>
        {recentDays.length === 0 ? (
          <p className="text-muted">
            No chapter has been completed yet. <Link to="/chapters">Pick one</Link> to
            get started.
          </p>
        ) : (
              ...
              <tbody>
                {recentDays.map((day) => (
                  <tr key={day.date}>
                    ...
                    <td>
                      {day.chapterNumbers
                        .map((number) => titleByNumber.get(number) ?? `Chapter ${number}`)
                        .join(', ')}
                    </td>
                  </tr>
                ))}
