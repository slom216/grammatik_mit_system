...
const files = readdirSync(CHAPTER_DIR)
  .filter((file) => /^chapter-\d{3}-.+\.ts$/.test(file) && !file.startsWith('chapter-000'))
  .sort();

const chapters: ChapterDefinition[] = [];
for (const file of files) {
  const module = (await import(pathToFileURL(`${CHAPTER_DIR.pathname}${file}`).href)) as Record<
    string,
    ChapterDefinition
  >;
...
const exerciseCount = chapters.reduce((sum, chapter) => sum + chapter.exercises.length, 0);
...
for (const [kind, group] of [...byKind.entries()].sort((a, b) => b[1].length - a[1].length)) {
  ...
  for (const finding of group.slice(0, 40)) {
    console.log(`  ch${finding.chapter} ${finding.exerciseIds.join(' / ')}: ${finding.detail}`);
  }
